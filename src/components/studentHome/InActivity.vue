      ...
      <el-table :data="corporationsList">
        ...
        <el-table-column label="查看详情">
          <template slot-scope="scope">
            <el-button
              type="primary"
              @click="showActivitySummary(scope.row.activityId,scope.row.name,scope.row.place,scope.row.description,scope.row.eventTime,scope.row.clubName,scope.row.adminName)"
            >查看</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="提交申请" :visible.sync="addDialogVisible" width="50%" center>
      ...
      <el-form :model="addForm" ref="addFormRef" label-width="150px">
        <el-form-item label="社团编号:" prop="corporationId">
          <el-input v-model="addForm.clubid" readonly="true" style="width:82%;"></el-input>
        </el-form-item>
        ...
        <el-form-item label="退社理由:" prop="reason">
          <el-input
            ...
            placeholder="请务必写明学号、专业和姓名,方便我们审核！"
            style="width:82%;"
          ></el-input>
        </el-form-item>
      </el-form>
  ...
  methods: {
    ...
    addApply() {
      this.$refs.addFormRef.validate(async (valid) => {
        ...
        console.log(clubId);
        let result = await this.$http.post(
          this.$api.StudentExitClub + "/" + clubId
        );
    ...
    async showActivitySummary(
      ...
      console.log(this.addForm.eventTime);
